...
import { getAllUserTasks, endTask } from "../services/UrlService";
...
export default function MyTasksComponent() {
  ...
  const handleComplete = async (taskId) => {
    if (!window.confirm("Вы уверены, что хотите завершить эту задачу?")) return;
    try {
        ...
        {currentTask && (
          <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
            <div className="bg-white rounded-2xl p-6 w-2/5 relative">
              ...
              <p className="mb-2">
                <strong>Исполнитель:</strong> {currentTask.assigneeName}
              </p>

              <p className="mb-2">
                <strong>Приоритет:</strong> {priorityMap[currentTask.priority]}
              </p>

              <p className="mb-2">
                <strong>Сроки:</strong> {currentTask.startDate} – {currentTask.dueDate}
              </p>
